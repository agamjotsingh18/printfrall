...
import { AddShoppingCart, Close, LocalOffer, Nature } from "@mui/icons-material";
...
const KraftPaperLabels = ({ addToCart }) => {
  ...
  const kraftPaperLabelsDetails = {
    name: "Custom Kraft Paper Labels",
    description:
      "Our Kraft Paper Labels are perfect for adding a rustic and eco-friendly touch to your product lines and bespoke packaging. Made from premium fibrous materials, they display a rich texture that emphasizes sustainable authenticity.",
    features: [
      "Eco-friendly, sustainable, and entirely biodegradable",
      "Sturdy adhesive back prevents curling on clear/solid objects",
      "Perfect matte base texture for high-resolution graphics",
      "Available in versatile custom sheet arrangements",
    ],
    materials: ["Standard Kraft Paper", "Premium Kraft Paper", "Recycled Kraft Paper"],
    images: [
      "kraft-paper-labels.png",
      "kraft-paper-labels-1.png",
      "kraft-paper-labels-2.png",
      "kraft-paper-labels-3.png"
    ],
  ...
  const handleAddToCart = () => {
    ...
    if (selectedOption === "Custom") {
      item = {
        name: kraftPaperLabelsDetails.name,
        image: getCdnImage(kraftPaperLabelsDetails.images[0], { width: 150, height: 150 }),
        description: kraftPaperLabelsDetails.description,
        features: kraftPaperLabelsDetails.features,
        tags: kraftPaperLabelsDetails.tags,
        selectedSize: `${customQuantity} labels`,
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
              {kraftPaperLabelsDetails.tags.map((tag, idx) => (
                <Chip
                  key={idx}
                  label={tag}
                  size="small"
                  icon={tag === "Eco-Friendly" ? <Nature fontSize="small" /> : <LocalOffer fontSize="small" />}
                  sx={{
            ...
            <Zoom>
              <img
                src={getCdnImage(kraftPaperLabelsDetails.images[activeImageIndex], { width: 600, height: 450 })}
                alt={`${kraftPaperLabelsDetails.name} primary view`}
                width="600"
                height="450"
                fetchpriority="high"
                style={{
            ...
            >
              {kraftPaperLabelsDetails.images.map((imageName, index) => (
                <Paper
                  key={index}
                ...
                >
                  <img
                    ...
                    alt={`${kraftPaperLabelsDetails.name} thumbnail view ${index + 1}`}
                    width="90"
                    height="90"
                    loading="lazy"
                    style={{
          ...
          {selectedOption === "Custom" && (
            <Box sx={{ mb: 4 }}>
              ...
              <TextField
                ...
                InputProps={{
                  endAdornment: <InputAdornment position="end">labels</InputAdornment>,
                }}
                sx={{
                  width: "200px",
              ...
              <Typography variant="caption" sx={{ display: "block", mt: 1, color: "#5a6e7a" }}>
                Unit price: ₹{unitPrice} per label
              </Typography>
            </Box>
          )}
