...
const FrontAdhesiveStickers = ({ addToCart }) => {
  ...
  const productDetails = {
    ...
    features: [
      "Material: High-quality Transparent Vinyl",
      "Adhesive on the front (design area) for glass surfaces",
      "Vibrant single-sided printing that stands out on clear glass",
      "Waterproof, durable, and versatile for all weather",
      "Delivered as easy-to-use sheets (Quantity refers to sticker count)",
      "Available in 7 different shapes and multiple sizes",
      "Order from just 50 units – ideal for sales and promotions",
    ],
  ...
  const handleAddToCart = () => {
    ...
    if (selectedOption === "Custom") {
      item = {
        ...
        selectedSize: `${customQuantity} stickers (${selectedShape})`,
            ...
            <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
              {productDetails.tags.map((tag, idx) => (
                <Chip
                  ...
                  label={tag}
                  size="small"
                  icon={tag === "Window Display" ? <Window fontSize="small" /> : <WorkspacePremium fontSize="small" />}
                  sx={{
          ...
          {selectedOption === "Custom" && (
            <Box sx={{ mb: 4 }}>
              ...
              <Typography variant="caption" sx={{ display: "block", mt: 1, color: "#5a6e7a" }}>
                Unit price: ₹{unitPrice.toFixed(2)} per sticker
              </Typography>
            </Box>
          )}
