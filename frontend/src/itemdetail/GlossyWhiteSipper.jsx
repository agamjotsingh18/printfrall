...
const GlossyWhiteSipper = ({ addToCart }) => {
  ...
  // Available options
  const availableColors = ["Glossy White"];
  const defaultSize = "400ml";
  const defaultColor = "Glossy White";
  ...
  const sipperDetails = {
    ...
    features: [
      "400 ML Capacity",
      "Smart grip-pad bottom prevents tipping",
      "Double-wall vacuum insulation (Hot/Cold up to 12h)",
      "Non-toxic double-layer stainless steel",
      "BPA-free and safety-certified",
      "Multicolour UV & Screen printing available",
      "Custom print area: 1.5 x 3 inches",
    ],
    sizes: ["400ml"],
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={6}>
          ...
          <Box sx={{ display: "flex", gap: 1, mb: 2, flexWrap: "wrap" }}>
            {sipperDetails.tags.map((tag, idx) => (
              <Chip
                ...
                label={tag}
                size="small"
                icon={tag === "Spill-Proof" ? <Security fontSize="small" /> : <Speed fontSize="small" />}
                sx={{
          ...
          <Typography variant="body2" sx={{ mt: 2, color: "#5a6e7a", fontStyle: "italic", textAlign: "center" }}>
            * Featuring PrintfrAll "Grip-Pad" Anti-Tip Technology.
          </Typography>
        </Grid>
      </Grid>
