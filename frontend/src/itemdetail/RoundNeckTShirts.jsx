...
const RoundNeckTShirts = ({ addToCart }) => {
  ...
  const priceMapping = {
    Cotton: 500,
    "Polyester Dri-Fit": 293,
    "Premium Blend": 450,
  };

  const availableColors = ["Black", "White", "Navy Blue", "Melange Grey", "Royal Blue", "Red", "Orange", "Yellow"];
  ...
  const productDetails = {
    ...
    features: [
      "High Quality Dri-Fit Polyester fabric",
      "Moisture-wicking and breathable material",
      "Customizable with high-res photos or text",
      "Durable print quality that resists fading",
      "Comfortable half-sleeve design",
      "Suitable for machine wash in cold water",
      "Department: Men's, Women's, and Youth sizes",
    ],
            ...
            <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
              {productDetails.tags.map((tag, idx) => (
                <Chip
                  ...
                  label={tag}
                  size="small"
                  icon={tag === "Moisture Wicking" ? <LocalFireDepartment fontSize="small" /> : <Checkroom fontSize="small" />}
                  sx={{
