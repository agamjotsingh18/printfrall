...
const EmbroideredPulloverHoodies = ({ addToCart }) => {
  const priceMapping = {
    "270 GSM Polycotton Fleece": 1250,
  };

  const availableColors = ["Black", "Gray", "Navy Blue", "Maroon", "White", "Irish Green"];
  const availableSizes = ["S", "M", "L", "XL", "XXL"];

  const defaultMaterial = "270 GSM Polycotton Fleece";
  ...
  const productDetails = {
    ...
    features: [
      "Material: 270 GSM Polycotton Fleece (Soft & Warm)",
      "Style: Regular fit, Long Sleeve with Hooded Neck",
      "Branding: High-quality Embroidery (up to 6 thread colors)",
      "Branding Area: 4 x 4 inches (Front / Left Chest)",
      "Durable: Fabric warranty up to 25 washes",
      "Artwork: Supports vector formats (AI, PDF, SVG, EPS)",
      "Professional: Ideal for corporate team gear and premium merch",
    ],
    tags: ["270 GSM", "Premium Embroidery", "Fleece Fabric"],
    ...
    <Container sx={{ py: 6, maxWidth: 1200, margin: "40px auto 0 auto", px: { xs: 2, md: 3 } }}>
      <Grid container spacing={4}>
            ...
            <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
              {productDetails.tags.map((tag, idx) => (
                <Chip
                  ...
                  label={tag}
                  size="small"
                  icon={tag === "270 GSM" ? <VerifiedUser fontSize="small" /> : <Checkroom fontSize="small" />}
                  sx={{
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            * Note: Embroidery does not support gradients or blends. Solid shapes and colors only.
          </Typography>
        </Grid>
      </Grid>
      ...
      <Snackbar
        ...
        message="✓ Embroidered Hoodie added to cart!"
