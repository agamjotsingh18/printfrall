...
const CustomPrintedPulloverHoodies = ({ addToCart }) => {
  const priceMapping = {
    "100% Cotton Premium": 1100,
    "Cotton-Poly Blend": 1000,
  };

  const availableColors = ["Black", "Gray", "Navy Blue", "Red", "Olive Green", "White"];
  const availableSizes = ["S", "M", "L", "XL", "XXL"];

  const defaultMaterial = "100% Cotton Premium";
  ...
  const productDetails = {
    ...
    features: [
      ...
      "Style: Regular fit, Hooded Neck with Long Sleeves",
      "Customizable with logo, text, or high-res photos",
      "Durable high-quality stitching and standard length",
      "Unisex design suitable for all casual and corporate needs",
      "Origin: Proudly made in India",
      "Perfect for gifting or team identity events",
    ],
    tags: ["100% Cotton", "Unisex Fit", "Breathable Fabric"],
            ...
            <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
              {productDetails.tags.map((tag, idx) => (
                <Chip
                  ...
                  label={tag}
                  size="small"
                  icon={tag === "100% Cotton" ? <VerifiedUser fontSize="small" /> : <Checkroom fontSize="small" />}
                  sx={{
