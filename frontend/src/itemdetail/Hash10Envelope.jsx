...
import { AddShoppingCart, Close, MarkEmailRead, WorkspacePremium } from "@mui/icons-material";
import Zoom from "react-medium-image-zoom";
import "react-medium-image-zoom/dist/styles.css";

// ========== MAIN PRODUCT IMAGE ==========
import mainImg from "../assets/envelope-10-long.png";

// ========== EXTRA ANGLES ==========
import img2 from "../assets/envelope-10-long.png";
import img3 from "../assets/envelope-10-long-1.png";
import img4 from "../assets/envelope-10-long-2.png";
import img5 from "../assets/envelope-10-long-3.png";

const Hash10Envelope = ({ addToCart }) => {
  const priceMapping = {
    "90 GSM Standard White": 5,    // per envelope
    "90 GSM Premium Ivory": 6.5,
    "90 GSM Eco-Recycled": 7.5,
  };

  const defaultMaterial = "90 GSM Standard White";
  const [selectedMaterial, setSelectedMaterial] = useState(defaultMaterial);
  const [mainImage, setMainImage] = useState(mainImg);
  const [snackbarOpen, setSnackbarOpen] = useState(false);

  // Custom quantity / pack options
  const unitPrice = priceMapping[selectedMaterial]; // per envelope
  const [selectedOption, setSelectedOption] = useState("Pack of 100");
  const [customQuantity, setCustomQuantity] = useState(100);

  const packOptions = [
    { label: "Pack of 50", value: "Pack of 50", quantity: 50, price: unitPrice * 50 * 0.98 },   // 2% discount
  ...
  const productDetails = {
    ...
    image: mainImg,
    description:
      "Cater to your professional business needs with personalised #10 envelopes. These standard long-sized envelopes are printed using high-quality offset technology on durable 90 GSM paper, ensuring your correspondence makes a premium impression.",
    features: [
      "Standard Long Size: 9.25\" x 4.25\" (Final Size)",
      "High-Quality Offset Print on 90 GSM paper",
      "Bleed Size: 11.71\" x 9.01\" for full-edge coverage",
      "Green Line Safety Area for perfect text alignment",
      "Available for online design or custom artwork upload",
      "Ideal for professional letters, invoices, and legal documents",
      "Compatible with standard business stationery",
    ],
    materials: ["90 GSM Standard White", "90 GSM Premium Ivory", "90 GSM Eco-Recycled"],
    extraImages: [img2, img3, img4, img5],
    tags: ["Business Standard", "Offset Printed", "90 GSM"],
  };
    ...
    <Container sx={{ py: 6, maxWidth: 1200, margin: "40px auto 0 auto", px: { xs: 2, md: 3 } }}>
      <Grid container spacing={4}>
            ...
            <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
              {productDetails.tags.map((tag, idx) => (
                <Chip
                  ...
                  label={tag}
                  size="small"
                  icon={tag === "90 GSM" ? <WorkspacePremium fontSize="small" /> : <MarkEmailRead fontSize="small" />}
                  sx={{
        ...
        <Grid item xs={12} md={6}>
          ...
          {selectedOption === "Custom" && (
            <Box sx={{ mb: 4 }}>
              ...
              <Typography variant="caption" sx={{ display: "block", mt: 1, color: "#5a6e7a" }}>
                Unit price: ₹{unitPrice.toFixed(2)} per envelope
              </Typography>
            </Box>
          )}
          ...
          >
            * Powered by PrintfrAll High-Resolution Offset Printing Technology.
          </Typography>
        </Grid>
      </Grid>
